#!/usr/bin/env python3
"""
After Effects Auto-Generator v2 pour RL PostFlow - Version Simple
Basé sur la version qui fonctionnait dans le commit cbda219
"""
...
class AfterEffectsGeneratorV2Simple:
    """Générateur automatique de projets After Effects v2 - Version Simple qui fonctionne."""
...
""")
        ...
        for plan in plans:
            plan_num = plan['plan_num']
            # Utiliser des slashes pour Mac (pas backslashes)
            plan_file = f"/Volumes/resizelab/o2b-undllm/2_IN/_FROM_EDIT/BY_SHOTS/UNDLM_{plan_num:05d}.mov"
    ...
    alert("ERREUR: Plan EDIT manquant - UNDLM_{plan_num:05d}.mov");
        ...
        for plan in plans:
            ...
            if plan_num in available_graded:
                # Utiliser des slashes pour Mac (pas backslashes)
                plan_file = f"/Volumes/resizelab/o2b-undllm/2_IN/_FROM_GRADING/BY_SHOTS/UNDLM_{plan_num:05d}.mov"
...
"""
    ...
    def generate_ae_script_v2(self, sequence_id: str, plans: List[Dict], available_graded: List[int]) -> str:
        ...
        # Construire le chemin de sauvegarde Mac proprement
        save_file_path = f"/Volumes/resizelab/o2b-undllm/3_PROJECTS/2_ANIM/SEQUENCES/{sequence_id}/_AE/{sequence_id}_01.aep"
...
var saveFile = new File("{save_file_path}");
...
"""
    ...
    def generate_for_sequence(self, sequence_id: str, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            print(f"\n🎬 Génération AE v2 Simple pour {sequence_id}")
            ...
            local_script_file = local_script_dir / f"RL_PostFlow_{sequence_id}_GENERATION_V2_SIMPLE.jsx"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Générateur After Effects v2 Simple pour RL PostFlow')
